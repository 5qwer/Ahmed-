...
const admin = require('firebase-admin'); // لربط Firebase Storage
const multer = require('multer'); // لرفع الملفات
const { v4: uuidv4 } = require('uuid'); // لإنشاء معرفات فريدة للملفات

const app = express();
// const port = process.env.PORT || 3000; // Vercel يحدد البورت تلقائياً في بيئة Serverless

// Middleware
app.use(cors());
app.use(express.json({ limit: '50mb' })); // زيادة حجم الطلب لملفات الفيديو
app.use(express.urlencoded({ extended: true, limit: '50mb' })); // لضمان قراءة البيانات المشفرة في URL
...
if (!MONGODB_URI) {
    ...
    // لا تخرج من العملية مباشرة في بيئة Serverless، فقط سجل الخطأ
    // process.exit(1);
}

// دالة الاتصال بقاعدة البيانات
const connectDB = async () => {
    if (mongoose.connection.readyState === 1) { // متصل بالفعل
    ...
    try {
        ...
        // throw err; // أعد رمي الخطأ إذا كنت تريد معالجته في مكان آخر
    }
};

// قم بالاتصال بقاعدة البيانات عند بدء تشغيل الوظيفة
connectDB();

// ----------------------------------------------------
// Firebase Admin SDK Initialization
// ----------------------------------------------------
// تأكد من وجود متغيرات البيئة FIREBASE_SERVICE_ACCOUNT و FIREBASE_STORAGE_BUCKET في Vercel
// ملف serviceAccountKey.json يجب أن يكون محتواه كسلسلة JSON في متغير البيئة FIREBASE_SERVICE_ACCOUNT
try {
    ...
    // إذا فشل Firebase، قد لا تعمل وظائف رفع الملفات
}
...
async function uploadFile(file, folder) {
    if (!file || !file.buffer) { // تأكد أن الملف موجود وأن به buffer
        console.error('No file or file buffer provided for upload.');
        return null;
    }
    try {
        ...
        await fileUpload.makePublic(); // اجعل الملف متاحاً للقراءة العامة
        return `https://storage.googleapis.com/${bucket.name}/${fileName}`;
    } catch (error) {
        console.error('Error uploading file to Firebase:', error);
        throw new Error('حدث خطأ أثناء رفع الملف إلى التخزين السحابي: ' + error.message);
    }
}
...
// ----------------------------------------------------
    ...
    const studentSchema = new mongoose.Schema({
        ...
        gradeLevel: { type: String, enum: ['first', 'second', 'third'], default: 'first' }, // تم تعيين قيمة افتراضية
        balance: { type: Number, default: 0 },
        points: { type: Number, default: 0 },
        isBanned: { type: Boolean, default: false },
        banReason: { type: String },
        bannedAt: { type: Date },
        bannedBy: { type: mongoose.Schema.Types.ObjectId, refPath: 'bannedByType' },
    ...
    const supportStaffSchema = new mongoose.Schema({
        ...
        email: { type: String, unique: true, required: true }, // إضافة البريد الإلكتروني
        phoneNumber: { type: String }, // إضافة رقم الهاتف
        isOnline: { type: Boolean, default: false },
        lastLogout: { type: Date },
        password: { type: String, required: true } // إضافة كلمة المرور
    }, { discriminatorKey: 'type' });
    ...
    const lessonSchema = new mongoose.Schema({
        ...
        grade: { type: String, enum: ['first', 'second', 'third'], required: true }, // تم تعديل 'all'
    ...
    const subscriptionSchema = new mongoose.Schema({
        ...
        includedLessons: [{ type: mongoose.Schema.Types.ObjectId, ref: 'Lesson' }],
        isActive: { type: Boolean, default: true },
        createdAt: { type: Date, default: Date.now }
    });
        ...
        if (name === correctTeacher.fullName && code === correctTeacher.code && phone === correctTeacher.phoneNumber) {
            ...
            if (!teacher) {
                teacher = new Teacher({
                    ...
                    teacherCode: code,
                    phoneNumber: phone,
                    password: await bcrypt.hash('teacher_default_password', 10),
                    type: 'teacher'
                });
                await teacher.save();
            }
    ...
    app.get('/api/lessons', async (req, res) => {
        ...
        } catch (error) => { // Fix this catch syntax
    ...
    app.get('/api/payment-methods', async (req, res) => {
        try {
            const methods = await PaymentMethod.find({}, { password: 0 });
