...
class EducationApp {
    ...
    handleSupportLogin() {
        ...
        console.log('Support login attempt:', { name, code });
        ...
        const support = supportStaff.find(s => s.name === name && s.code === code);

        console.log('Support staff:', supportStaff);
        console.log('Found support:', support);
        ...
        if (support) {
            ...
            // تسجيل وقت الدخول
            support.lastLogin = new Date().toISOString();
    ...
    handleStudentRegistration() {
        ...
        // التحقق من عدم وجود طالب بنفس الاسم أو رقم الطالب
    ...
    showPage(pageId) {
        ...
        if (pageId === 'paidLessons') {
            ...
            this.loadStudentChatList();
        } else if (pageId === 'transferOperations') {
            this.loadTransferOperations();
        }
    }
    ...
    async showLessonMaterials(lesson) {
        ...
        if (solutionPdfUrl || solutionVideoUrl) {
            ...
            if(solutionVideoUrl && viewHomeworkBtn) {
                viewHomeworkBtn.style.display = 'inline-block';
                viewHomeworkBtn.onclick = () => this.showVideoModal(solutionVideoUrl, 'فيديو حل الواجب');
            } else {
                viewHomeworkBtn.style.display = 'none';
            }
    ...
    async handleLessonUpload() {
        ...
        this.loadTeacherLessons(); // تحديث القائمة
    }
        ...
        container.innerHTML = subscriptions.map(sub => `
            <div class="subscription-item">
                <div>
                    ...
                    <small>${sub.price} جنيه - ${sub.duration} يوم</small>
                </div>
    ...
    loadStudentMessages() {
        ...
        if (statusFilter !== 'all') {
            if (statusFilter === 'read') {
                 messages = messages.filter(m => m.isRead && !m.replies?.length);
            } else if (statusFilter === 'unread') {
                 messages = messages.filter(m => !m.isRead);
            } else if (statusFilter === 'replied') {
                 messages = messages.filter(m => m.replies && m.replies.length > 0);
            }
        }
        ...
        container.innerHTML = messages.sort((a,b) => new Date(b.timestamp) - new Date(a.timestamp)).map(msg => {
             ...
             if (msg.replies && msg.replies.length > 0) statusClass = 'replied';
             else if (msg.isRead) statusClass = 'read';
            ...
            <div class="message-item ${statusClass}">
                <div>
                    ...
                    <small>الحصة: ${msg.lessonTitle || 'غير محدد'}</small><br>
    ...
    async openReplyModal(messageId) {
        ...
        if (!message) {
            this.showMessage('لم يتم العثور على الرسالة.', 'error');
            return;
        }
        ...
        if (!message.isRead) {
            message.isRead = true;
            this.saveStoredData('studentMessages', messages);
            this.loadStudentMessages(); // Refresh the list in the background
        }

        const modal = document.getElementById('replyModal');
        const originalMessageContainer = document.getElementById('originalMessage');
        ...
        if (message.imageKey) {
            const imageUrl = await this.fileStore.getFileAsURL(message.imageKey);
            if (imageUrl) {
                imageHtml = `<img src="${imageUrl}" alt="صورة مرفقة" class="chat-image" style="max-width: 100%; border-radius: 8px; margin-top: 10px;">`;
            }
        }

        originalMessageContainer.innerHTML = `
            <p><strong>من:</strong> ${message.studentName}</p>
            <p><strong>الموضوع:</strong> ${message.subject}</p>
            <p><strong>الرسالة:</strong> ${message.text}</p>
            ${imageHtml}
            <small>الوقت: ${new Date(message.timestamp).toLocaleString('ar-EG')}</small>
        `;
    ...
    async startRecording() {
        try {
            ...
            this.showMessage('بدء التسجيل...', 'info');

        } catch (err) {
            console.error('Error accessing microphone:', err);
            this.showMessage('لا يمكن الوصول إلى الميكروفون. يرجى التأكد من منح الإذن.', 'error');
        }
    }
    ...
    stopRecording() {
        if (this.mediaRecorder && this.mediaRecorder.state === 'recording') {
            ...
            this.showMessage('تم إيقاف التسجيل.', 'success');
        }
    }
    ...
    async handleReplySubmit() {
        ...
        if (!replyText && !replyImageInput.files[0] && !this.recordedBlob) {
            this.showMessage('يجب كتابة رد أو إرفاق صورة أو تسجيل صوتي.', 'error');
            return;
        }

        let imageKey = null;
        if (replyImageInput.files[0]) {
            imageKey = await this.handleFileUpload('replyImage', `reply_${messageId}_${Date.now()}`);
        }
        
        let audioKey = null;
        if (this.recordedBlob) {
             audioKey = `reply_audio_${messageId}_${Date.now()}`;
             await this.fileStore.saveFile(audioKey, this.recordedBlob);
        }
        ...
        const messages = this.getStoredData('studentMessages') || [];
        ...
        if (messageIndex !== -1) {
            ...
            messages[messageIndex].status = 'replied'; // Update status
            this.saveStoredData('studentMessages', messages);

            // Send notification to student
            this.addNotificationToStudent(
                messages[messageIndex].studentId,
                'teacher',
                'رد من المدرس',
                `قام المدرس بالرد على سؤالك بخصوص: "${messages[messageIndex].subject}"`,
                true
            );

            this.showMessage('تم إرسال الرد بنجاح!', 'success');
            this.closeReplyModal();
            this.loadStudentMessages();
        } else {
            this.showMessage('خطأ: لم يتم العثور على الرسالة الأصلية.', 'error');
        }
    }
    ...
    // تحميل صفحة عرض عمليات التحويل للمدرس
    loadTransferOperations() {
        ...
        // حساب الإحصائيات
        const totalAmount = transferRequests.reduce((sum, req) => sum + req.amount, 0);
        const confirmedRequests = transferRequests.filter(req => req.status === 'confirmed').length;
        const pendingRequests = transferRequests.filter(req => req.status === 'pending').length;
        
        // تحديث الإحصائيات
        this.updateElement('totalTransfers', `${totalAmount} جنيه`);
        this.updateElement('confirmedTransfers', confirmedRequests);
        this.updateElement('pendingTransfers', pendingRequests);
        ...
        if (transferRequests.length === 0) {
            container.innerHTML = '<p>لا توجد عمليات تحويل حتى الآن</p>';
            return;
        }
        ...
        container.innerHTML = transferRequests
            .sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp))
            .map(request => {
                const paymentMethods = this.getStoredData('paymentMethods') || [];
                const paymentMethod = paymentMethods.find(pm => pm.id == request.paymentMethodId);
                const methodName = paymentMethod ? paymentMethod.name : 'غير محدد';
                
                return `
                    <div class="transfer-item">
                        <div class="transfer-info">
                            <strong>${request.studentName}</strong>
                            <small>المبلغ: ${request.amount} جنيه</small>
                            <small>وسيلة الدفع: ${methodName}</small>
                            <small>رقم العملية: ${request.transactionNumber}</small>
                            <small>الحالة: <span class="${statusClass[request.status]}">${statusText[request.status]}</span></small>
                            <small>تاريخ الطلب: ${new Date(request.timestamp).toLocaleDateString('ar-EG')}</small>
                            ${request.rejectionReason ? `<small>سبب الرفض: ${request.rejectionReason}</small>` : ''}
                        </div>
                        <div class="transfer-actions">
                            ${request.receiptImageKey ? `
                                <button class="view-btn" onclick="window.app.viewReceipt('${request.receiptImageKey}')">
                                    <i class="fas fa-eye"></i>
                                    عرض الإيصال
                                </button>
                            ` : ''}
...
// Implementation for missing methods from previous prompts that might be related
...
EducationApp.prototype.searchAndFilterStudents = function(role) {
    ...
    resultsContainer.innerHTML = students.map(student => {
        const statusClass = student.isBanned ? 'banned' : 'active';
        return `
            <div class="student-result ${statusClass}">
                <div>
...
EducationApp.prototype.viewReceipt = async function(receiptKey) {
    ...
    if (!imageUrl) {
        this.MenuItem('لا توجد صورة إيصال', 'error');
        return;
    }
...
// تحميل عمليات الدفع المعلقة
EducationApp.prototype.loadPendingPayments = function() {
    if (!this.currentUser || this.currentUser.type !== 'support') return;
    ...
    if (transferRequests.length === 0) {
        container.innerHTML = '<p>لا توجد عمليات دفع معلقة</p>';
        return;
    }
                ...
                <div class="payment-item">
                    ...
                    <div class="payment-actions">
                        ${request.status === 'pending' ? `
                            <button class="confirm-btn" onclick="window.app.confirmPayment(${request.id})">
                                <i class="fas fa-check"></i>
                                تأكيد
                            </button>
                            <button class="reject-btn" onclick="window.app.rejectPayment(${request.id})">
...
// تأكيد عملية الدفع
EducationApp.prototype.confirmPayment = function(paymentId) {
    if (!this.currentUser || this.currentUser.type !== 'support') return;
    
    if (!confirm('هل أنت متأكد من تأكيد هذه العملية؟ سيتم إضافة المبلغ لرصيد الطالب.')) return;
    
    const transferRequests = this.getStoredData('transferRequests') || [];
    const paymentIndex = transferRequests.findIndex(r => r.id === paymentId);
    
    if (paymentIndex === -1) {
        this.showMessage('العملية غير موجودة', 'error');
        return;
    }
    
    const payment = transferRequests[paymentIndex];
    
    // تحديث حالة الطلب
    transferRequests[paymentIndex].status = 'confirmed';
    transferRequests[paymentIndex].confirmedBy = this.currentUser.id;
    transferRequests[paymentIndex].confirmedAt = new Date().toISOString();
    this.saveStoredData('transferRequests', transferRequests);
    
    // إضافة المبلغ لرصيد الطالب
    const students = this.getStoredData('students') || [];
    const studentIndex = students.findIndex(s => s.id === payment.studentId);
    
    if (studentIndex !== -1) {
        students[studentIndex].balance = (students[studentIndex].balance || 0) + payment.amount;
        students[studentIndex].lastActivity = new Date().toISOString();
        this.saveStoredData('students', students);
        ...
        this.addNotificationToStudent(
            ...
            'payment',
            'تأكيد التحويل',
            `تم تأكيد تحويلك بقيمة ${payment.amount} جنيه وإضافته لرصيدك`,
            false
        );
    }
    ...
    this.showMessage('تم تأكيد العملية بنجاح', 'success');
    this.loadPendingPayments();
}

// رفض عملية الدفع
EducationApp.prototype.rejectPayment = function(paymentId) {
    const reason = prompt("يرجى إدخال سبب الرفض:");
    if (!reason || reason.trim() === '') {
        this.showMessage('يجب إدخال سبب الرفض', 'error');
        return;
    }
    ...
    // تحديث حالة الطلب
    transferRequests[paymentIndex].status = 'rejected';
    transferRequests[paymentIndex].rejectedBy = this.currentUser.id;
    transferRequests[paymentIndex].rejectedAt = new Date().toISOString();
    transferRequests[paymentIndex].rejectionReason = reason;
    this.saveStoredData('transferRequests', transferRequests);
    ...
    this.addNotificationToStudent(
        ...
        'payment',
        'رفض التحويل',
        `تم رفض طلب التحويل بقيمة ${payment.amount} جنيه. السبب: ${reason}`,
        false
    );
    ...
    this.showMessage('تم رفض العملية', 'success');
    this.loadPendingPayments();
}

// فلترة عمليات الدفع
EducationApp.prototype.filterPayments = function(filterType) {
    // تحديث حالة الأزرار
    document.querySelectorAll('#confirmPayments .filter-btn').forEach(btn => {
        btn.classList.remove('active');
    });
    ...
    switch(filterType) {
        case 'all':
            filteredPayments = transferRequests;
            break;
        case 'pending':
            filteredPayments = transferRequests.filter(r => r.status === 'pending');
            break;
        case 'today':
            const today = new Date().toDateString();
            filteredPayments = transferRequests.filter(r => 
...
// عرض عمليات الدفع المفلترة
EducationApp.prototype.displayFilteredPayments = function(payments) {
    const container = document.getElementById('pendingPaymentsList');
    if (!container) return;
    
    if (payments.length === 0) {
        container.innerHTML = '<p>لا توجد عمليات تطابق المعايير المحددة</p>';
        return;
    }
        ...
        return `
            <div class="payment-item">
                <div class="payment-info">
                    ...
                    <small>المبلغ: ${payment.amount} جنيه</small>
                    <small>وسيلة الدفع: ${methodName}</small>
                    <small>رقم العملية: ${payment.transactionNumber}</small>
                    <small>الحالة: <span class="${statusClass[payment.status]}">${statusText[payment.status]}</span></small>
                    <small>تاريخ الطلب: ${new Date(payment.timestamp).toLocaleDateString('ar-EG')}</small>
                    ${payment.rejectionReason ? `<small>سبب الرفض: ${payment.rejectionReason}</small>` : ''}
                </div>
                <div class="payment-actions">
                    ${payment.status === 'pending' ? `
                        <button class="confirm-btn" onclick="window.app.confirmPayment(${payment.id})">
                            <i class="fas fa-check"></i>
                            تأكيد
                        </button>
                        <button class="reject-btn" onclick="window.app.rejectPayment(${payment.id})">
                            <i class="fas fa-times"></i>
                            رفض
                        </button>
                    ` : ''}
                    ${payment.receiptImageKey ? `
                        <button class="view-btn" onclick="window.app.viewReceipt('${payment.receiptImageKey}')">
                            <i class="fas fa-eye"></i>
                            عرض الإيصال
                        </button>
                    ` : ''}
...
// إضافة إشعار للطالب
EducationApp.prototype.addNotificationToStudent = function(studentId, type, title, message, canReply = false) {
    const notifications = this.getStoredData('studentNotifications') || [];
    notifications.push({
        id: Date.now(),
        studentId: studentId,
...
// تحميل عمليات الطلاب
EducationApp.prototype.loadStudentOperations = function() {
    if (!this.currentUser || this.currentUser.type !== 'support') return;
    ...
    const studentsWithBalance = students.filter(s => (s.balance || 0) > 0).length;
    ...
    const activeStudents = students.filter(s => 
        s.lastActivity && new Date(s.lastActivity) > recentActivityThreshold
    ).length;
    
    // تحديث الإحصائيات
    this.updateElement('totalStudentsCount', totalStudents);
    this.updateElement('studentsWithBalance', studentsWithBalance);
    this.updateElement('totalBalance', `${totalBalance} جنيه`);
    this.updateElement('activeStudents', activeStudents);
    
    // عرض جميع الطلاب بشكل افتراضي
    this.filterStudentsByBalance('all');
}

// فلترة الطلاب حسب الرصيد
EducationApp.prototype.filterStudentsByBalance = function(filterType) {
    // تحديث حالة الأزرار
    document.querySelectorAll('#studentOperations .filter-btn').forEach(btn => {
        btn.classList.remove('active');
    });
    event.target.classList.add('active');
    ...
    switch(filterType) {
        case 'all':
            ...
            break;
        case 'hasBalance':
            filteredStudents = students.filter(s => (s.balance || 0) > 0);
            break;
        case 'noBalance':
            filteredStudents = students.filter(s => (s.balance || 0) === 0);
            break;
        case 'recentActivity':
            ...
            filteredStudents = students.filter(s => 
                s.lastActivity && new Date(s.lastActivity) > weekAgo
            );
            break;
    }
    
    this.displayStudentOperations(filteredStudents);
}

// عرض عمليات الطلاب
EducationApp.prototype.displayStudentOperations = function(students) {
    const container = document.getElementById('studentOperationsList');
    if (!container) return;
    
    if (students.length === 0) {
        container.innerHTML = '<p>لا يوجد طلاب يطابقون المعايير المحددة</p>';
        return;
    }
    ...
    container.innerHTML = students.map(student => {
        ...
        const lastActivity = student.lastActivity ? 
            new Date(student.lastActivity).toLocaleDateString('ar-EG') : 'لا يوجد';
        
        return `
            <div class="operation-item">
                <div class="operation-info">
                    <strong>${student.name}</strong>
                    <small>الصف: ${this.getGradeText(student.grade)}</small>
                    <small>رقم الطالب: ${student.studentNumber}</small>
                    <small>الرصيد: ${balance} جنيه</small>
                    <small>آخر نشاط: ${lastActivity}</small>
                    <small>النقاط: ${student.points || 0} نقطة</small>
                    ${student.isBanned ? '<small style="color: var(--accent-color);">محظور</small>' : ''}
                </div>
                <div class="operation-actions">
                    <button class="view-btn" onclick="window.app.viewStudentDetails(${student.id})">
                        <i class="fas fa-eye"></i>
                        عرض التفاصيل
                    </button>
                </div>
